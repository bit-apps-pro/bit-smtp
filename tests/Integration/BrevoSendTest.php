...
/**
 * Drives wp_mail() through the Brevo API transport (mocking the outbound HTTP call) and proves
...
final class BrevoSendTest extends IntegrationTestCase
{
    public function testSendsSuccessfullyWhenTheApiKeyIsEncryptedAtRest(): void
    {
        ...
        try {
            ...
            $sent = wp_mail('to@example.org', 'Encrypted Brevo Send', 'Body');
        } finally {
            remove_filter('pre_http_request', $filter, 10);
        }

        $this->assertBrevoRequestCarriedTheApiKey($sent, $captured, $apiKey, 'Encrypted Brevo Send');
    }
    ...
    public function testSendsSuccessfullyWhenTheApiKeyIsStoredAsLegacyPlaintext(): void
    {
        ...
        try {
            ...
            $sent = wp_mail('to@example.org', 'Legacy Plaintext Brevo Send', 'Body');
        } finally {
            remove_filter('pre_http_request', $filter, 10);
        }

        $this->assertBrevoRequestCarriedTheApiKey($sent, $captured, $apiKey, 'Legacy Plaintext Brevo Send');
    }

    /**
     * @param array<string,mixed> $captured
     */
    private function assertBrevoRequestCarriedTheApiKey(bool $sent, array $captured, string $apiKey, string $subject): void
    {
        $this->assertTrue($sent, 'wp_mail() should report the Brevo send as successful');
        $this->assertNotEmpty($captured, 'the Brevo transport should have issued an outbound HTTP request');
        $this->assertStringContainsString(
            $apiKey,
            (string) wp_json_encode($captured['headers']),
            'load() must decrypt the stored api_key back to plaintext for the api-key header'
        );
    ...
    private function v2(string $apiKey): array
    {
        return [
            'schema_version'          => 2,
            'enabled'                 => true,
            'default_connection_id'   => 'conn_brevo',
     ...
     * @param array<string,mixed> $captured
     */
    private function interceptBrevoRequest(array &$captured): callable
    {
        return static function ($preempt, $args, $url) use (&$captured) {
            if (strpos($url, 'api.brevo.com') === false) {
                return $preempt;
            }
